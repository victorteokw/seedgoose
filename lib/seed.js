...
const seed = async ({ mongoose, report, args, options }) => {
  ...
  for (const collection of collections) {
    ...
    const model = mongoose.models.find((m) => m.collection.name === collection);
...
const replaceRefsOnRecord = async ({
  record, treeSchema, collectionName, models, savedPath
}) => {
  await eachAsync(treeSchema, async (schemaItem, key) => {
    ...
    if (schemaItem instanceof mongoose.Schema) {
      await replaceRefsOnRecord(
        record, schemaItem.tree, collectionName, models, [...savedPath, key]
      );
      ...
      if ((schemaItem.type === mongoose.Schema.Types.ObjectId) ||
        ...
        // Nested objects
        await replaceRefsOnRecord(
          record, schemaItem, collectionName, models, [...savedPath, key]
        );
      }
    // array
...
const seedCollection = async ({
  ...
  for (const record of data) {
    ...
    console.log(`[${collection}] ${update ? 'update': 'create'} ${smartId}`);
  }
};
