    ...
    await eachAsync((schema as any).tree, async (item, key) => {
      if (record && record[key]) {
        if (!shouldIgnoreItem(item, key, mongoose)) {
          retval[key] = await transformRecord(
            record[key],
            item,
            mongoose,
            root
  ...
  if (typeof schema === "object") {
    ...
    if (schema.type) {
      if (
        ...
        schema.type === mongoose.Types.ObjectId
      ) {
        if (stringIsObjectId(record)) return mongoose.Types.ObjectId(record);
        // Reference type
        if (schema.ref) {
          // Static type reference
          return await getUniqId(
            mongoose.model(schema.ref).collection.name,
            record
          );
        } else if (schema.refPath) {
          // Dynamic type reference
          return await getUniqId(
            mongoose.model(get(root, schema.refPath)).collection.name,
            record
          );
        } else {
    ...
    if (command === SeedingCommandType.SEED) {
      ...
      if (exist) {
        ...
        if (insertResult.result.ok) {
          ...
          throw new Error(
            `cannot insert ${collectionName} data of ${record._id}`
          );
        }
        return;
      }
